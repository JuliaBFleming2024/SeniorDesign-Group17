...
def get_start_end_times( event_time_list, event_rows ):
    ...
    for x in range(event_rows):
        ...
        # Place None for events missing time info.
        if len(dayTimeSplit)    == 1:
            event_time_start[x] = None
            event_time_end[x]   = None

        # has 2 splits, first element is the day, 2nd element is the time.
        ...
        # has 3 splits, 1st mmm dd, 2nd time - mmm dd, 3rd time
        elif len(dayTimeSplit)  == 3:
            # FIXME - if there are 2 times listed, use those as start and end. Should be easy, just using None until it gets fixed.
            event_time_start[x] = None
            event_time_end[x]   = None

        # otherwise, use None to at least have it show in the "missing time info" dataframe
        else:
            event_time_start[x] = None
            event_time_end[x]   = None
...
# process date column to focus on times
def process_dates( time_list ):
    days    = re.compile('(Sun|Mon|Tue|Wed|Thu|Fri|Sat), ')
    tz_abbr = re.compile(' (AST|EST|MST|ET|CT|UTC|EDT|WST|CST|PST|NST)')
    for x in range(len(time_list)):
        time_list[x] = days.sub('', time_list[x])
        time_list[x] = tz_abbr.sub('', time_list[x])
    return time_list
# End process_dates
...
def standardize_time( time2Adj ):
    if (len(time2Adj) == 1):
        fixTime = time2Adj[0].split(":")
        if ( len(fixTime) == 1 ):
            ampmTime = fixTime[0].split(" ")
            time2Adj[0] = ampmTime[0] + ":00 " + ampmTime[1]
        return time2Adj
        ...
        if ( ("AM" not in time2Adj[0]) and ("PM" not in time2Adj[0]) ):
...
def itinerary_generator( event_results_df, maps_results_df, num_events, num_restaurants, itin_start_time ):
    ...
    event_time_list = process_dates(event_time_list)
...
# FIXME - need to process input to accept time written in any format.
...
if st.button("Search my Events ", type="primary"):
    try:
        ...
        for page in range(2):
            ...
            event_results = event_results.get("events_results", [])
            events_results = events_results + event_results
        ...
        generated_itineray_events, random_restaurants, event_results_WO_time_data_df = itinerary_generator( event_results_df, maps_results_df, num_events, num_restaurants, itin_start_time  )
